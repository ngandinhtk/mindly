export const quotes = [
  {
    text: "Hạnh phúc không phải là đích đến, mà là cách ta đi",
    author: "Thích Nhất Hạnh"
  },
  {
    text: "Mỗi ngày là một trang mới trong cuốn sách cuộc đời của bạn",
    author: "Unknown"
  },
  {
    text: "Nụ cười của bạn có thể thay đổi cả ngày của ai đó",
    author: "Unknown"
  },
  {
    text: "Hôm nay khó khăn, ngày mai sẽ tốt hơn",
    author: "Unknown"
  },
  {
    text: "Bình an đến từ bên trong, đừng tìm kiếm nó từ bên ngoài",
    author: "Buddha"
  },
  {
    text: "Mỗi cảm xúc đều có giá trị của nó, hãy đón nhận và thấu hiểu",
    author: "Unknown"
  },
  {
    text: "Hạnh phúc nhỏ nhất cũng là hạnh phúc",
    author: "Unknown"
  },
  {
    text: "Mọi việc sẽ ổn thôi, tin vào điều đó",
    author: "Unknown"
  },
  {
    text: "Yêu thương bản thân là khởi đầu của một cuộc phiêu lưu suốt đời",
    author: "Oscar Wilde"
  },
  {
    text: "Đừng để ngày hôm qua chiếm quá nhiều thời gian của ngày hôm nay",
    author: "Will Rogers"
  },
  {
    text: "Cuộc sống là 10% điều xảy ra với bạn và 90% là cách bạn phản ứng với nó",
    author: "Charles R. Swindoll"
  },
  {
    text: "Hãy là phiên bản tốt nhất của chính mình",
    author: "Unknown"
  },
  {
    text: "Đôi khi những điều tồi tệ nhất lại dẫn đến những điều tốt đẹp nhất",
    author: "Unknown"
  },
  {
    text: "Giọt nước có thể làm tràn ly, nhưng cũng có thể làm nở hoa",
    author: "Unknown"
  },
  {
    text: "Mỗi bước đi nhỏ đều đưa ta đến gần hơn với mục tiêu",
    author: "Unknown"
  },
  {
    text: "Khi bạn thay đổi cách nhìn về mọi thứ, những thứ bạn nhìn cũng sẽ thay đổi",
    author: "Wayne Dyer"
  },
  {
    text: "Đừng so sánh bản thân với người khác, hãy so sánh với chính mình của ngày hôm qua",
    author: "Unknown"
  },
  {
    text: "Điều quan trọng không phải là vấp ngã mấy lần, mà là đứng dậy được bao nhiêu lần",
    author: "Unknown"
  },
  {
    text: "Mỗi ngày đều là cơ hội để bắt đầu lại",
    author: "Unknown"
  },
  {
    text: "Hãy dành thời gian để làm những điều khiến tâm hồn bạn hạnh phúc",
    author: "Unknown"
  },
  {
    text: "Sống chậm lại, cảm nhận nhiều hơn",
    author: "Unknown"
  },
  {
    text: "Không có cảm xúc nào là sai, chỉ có cách ta phản ứng với nó",
    author: "Unknown"
  },
  {
    text: "Mỗi hơi thở là một món quà của cuộc sống",
    author: "Thích Nhất Hạnh"
  },
  {
    text: "Hạnh phúc đến từ việc biết đủ",
    author: "Unknown"
  },
  {
    text: "Bạn không thể thay đổi quá khứ, nhưng bạn có thể bắt đầu thay đổi từ bây giờ",
    author: "Unknown"
  },
  { text: "Cách tốt nhất để dự đoán tương lai là tạo ra nó.", author: "Peter Drucker" },
  { text: "Bạn không bao giờ quá già để đặt ra một mục tiêu khác hoặc mơ một giấc mơ mới.", author: "C.S. Lewis" },
  { text: "Hành trình ngàn dặm bắt đầu từ một bước chân.", author: "Lão Tử" },
  { text: "Những gì bạn nhận được khi đạt được mục tiêu không quan trọng bằng con người bạn trở thành khi đạt được mục tiêu.", author: "Zig Ziglar" },
  { text: "Giới hạn duy nhất cho việc hiện thực hóa ngày mai của chúng ta sẽ là những nghi ngờ của chúng ta về ngày hôm nay.", author: "Franklin D. Roosevelt" },
  { text: "Không quan trọng bạn đi chậm như thế nào miễn là bạn không dừng lại.", author: "Khổng Tử" },
  { text: "Mọi thứ bạn từng mong muốn đều ở phía bên kia của sự sợ hãi.", author: "George Addair" },
  { text: "Hãy tin vào bản thân và tất cả những gì bạn đang có. Biết rằng có một cái gì đó bên trong bạn vĩ đại hơn bất kỳ trở ngại nào.", author: "Christian D. Larson" },
  { text: "Bí quyết để tiến lên là bắt đầu.", author: "Mark Twain" },
  { text: "Ý chí để chiến thắng, khao khát để thành công, sự thôi thúc để đạt được tiềm năng tối đa của bạn... đây là những chiếc chìa khóa sẽ mở ra cánh cửa đến với sự xuất sắc của cá nhân.", author: "Khổng Tử" },
  { text: "Thời gian của bạn có hạn, đừng lãng phí nó để sống cuộc đời của người khác.", author: "Steve Jobs" },
  { text: "Hai ngày quan trọng nhất trong cuộc đời bạn là ngày bạn được sinh ra và ngày bạn tìm ra lý do tại sao.", author: "Mark Twain" },
  { text: "Người duy nhất bạn được định sẵn để trở thành là người mà bạn quyết định trở thành.", author: "Ralph Waldo Emerson" },
  { text: "Hãy tự tin đi theo hướng ước mơ của bạn. Sống cuộc sống mà bạn đã tưởng tượng.", author: "Henry David Thoreau" },
  { text: "Khi tôi buông bỏ con người của mình, tôi trở thành con người mà tôi có thể trở thành.", author: "Lão Tử" },
  { text: "Bạn là chủ nhân của số phận của mình. Bạn có thể ảnh hưởng, chỉ đạo và kiểm soát môi trường của riêng mình. Bạn có thể biến cuộc sống của mình thành những gì bạn muốn.", author: "Napoleon Hill" },
  { text: "Hãy xây dựng ước mơ của riêng bạn, hoặc người khác sẽ thuê bạn để xây dựng ước mơ của họ.", author: "Farrah Gray" },
  { text: "Tương lai phụ thuộc vào những gì bạn làm hôm nay.", author: "Mahatma Gandhi" },
  { text: "Tâm trí là tất cả. Bạn nghĩ gì bạn sẽ trở thành như vậy.", author: "Phật" },
  { text: "Hoặc bạn điều hành một ngày, hoặc một ngày điều hành bạn.", author: "Jim Rohn" },
  { text: "Tôi không thất bại. Tôi chỉ tìm ra 10.000 cách không hoạt động.", author: "Thomas A. Edison" },
  { text: "Nếu bạn không sẵn sàng mạo hiểm những điều thông thường, bạn sẽ phải chấp nhận những điều bình thường.", author: "Jim Rohn" },
  { text: "Tất cả những giấc mơ của chúng ta đều có thể trở thành sự thật, nếu chúng ta có can đảm để theo đuổi chúng.", author: "Walt Disney" },
  { text: "Nếu bạn làm những gì bạn đã luôn làm, bạn sẽ nhận được những gì bạn đã luôn nhận được.", author: "Tony Robbins" },
  { text: "Sự trả thù tốt nhất là thành công vang dội.", author: "Frank Sinatra" },
  { text: "Tôi không phải là sản phẩm của hoàn cảnh của tôi. Tôi là sản phẩm của những quyết định của tôi.", author: "Stephen Covey" },
  { text: "Không có giới hạn nào cho những gì bạn có thể hoàn thành, ngoại trừ những giới hạn bạn đặt ra cho suy nghĩ của chính mình.", author: "Brian Tracy" },
  { text: "Một người đàn ông thành công là người có thể đặt một nền móng vững chắc bằng những viên gạch mà người khác đã ném vào anh ta.", author: "David Brinkley" },
  { text: "Không ai có thể khiến bạn cảm thấy thấp kém nếu không có sự đồng ý của bạn.", author: "Eleanor Roosevelt" },
  { text: "Câu hỏi không phải là ai sẽ cho phép tôi; mà là ai sẽ ngăn cản tôi.", author: "Ayn Rand" },
  { text: "Nếu bạn muốn nâng mình lên, hãy nâng người khác lên.", author: "Booker T. Washington" },
  { text: "Những gì đối với chúng ta là những thử thách cay đắng thường là những phước lành trá hình.", author: "Oscar Wilde" },
  { text: "Khoảng cách giữa sự điên rồ và thiên tài chỉ được đo bằng thành công.", author: "Bruce Feirstein" },
  { text: "Khi bạn ngừng theo đuổi những điều sai trái, bạn sẽ cho những điều đúng đắn một cơ hội để bắt kịp bạn.", author: "Lolly Daskal" },
  { text: "Tôi không thể thay đổi hướng gió, nhưng tôi có thể điều chỉnh cánh buồm của mình để luôn đến được đích.", author: "Jimmy Dean" },
  { text: "Di chuyển nhanh và phá vỡ mọi thứ. Trừ khi bạn đang phá vỡ mọi thứ, bạn không di chuyển đủ nhanh.", author: "Mark Zuckerberg" },
  { text: "Không phải gánh nặng làm bạn gục ngã, mà là cách bạn mang nó.", author: "Lou Holtz" },
  { text: "Tôi càng làm việc chăm chỉ, tôi càng may mắn.", author: "Gary Player" },
  { text: "Điều duy nhất đứng giữa bạn và mục tiêu của bạn là câu chuyện nhảm nhí mà bạn không ngừng kể cho bản thân nghe về lý do tại sao bạn không thể đạt được nó.", author: "Jordan Belfort" },
  { text: "Nếu bạn đang làm việc gì đó mà bạn thực sự quan tâm, bạn không cần phải bị thúc ép. Tầm nhìn sẽ kéo bạn đi.", author: "Steve Jobs" },
  { text: "Điều về những khởi đầu mới là chúng đòi hỏi một cái gì đó khác phải kết thúc.", author: "Unknown" },
  { text: "Và đột nhiên bạn biết: Đã đến lúc bắt đầu một cái gì đó mới và tin tưởng vào sự kỳ diệu của những khởi đầu.", author: "Meister Eckhart" },
  { text: "Bước đầu tiên để đến một nơi nào đó là quyết định rằng bạn sẽ không ở lại nơi bạn đang ở.", author: "J.P. Morgan" },
  { text: "Có những điều tốt đẹp hơn nhiều ở phía trước hơn bất kỳ điều gì chúng ta để lại phía sau.", author: "C.S. Lewis" },
  { text: "Bí quyết để thay đổi là tập trung tất cả năng lượng của bạn, không phải vào việc chống lại cái cũ, mà là vào việc xây dựng cái mới.", author: "Socrates" },
  { text: "Tôi phát hiện ra rằng một khởi đầu mới là một quá trình. Một khởi đầu mới là một cuộc hành trình - một cuộc hành trình đòi hỏi một kế hoạch.", author: "Unknown" },
  { text: "Hoàn cảnh hiện tại của bạn không quyết định nơi bạn có thể đến; chúng chỉ đơn thuần xác định nơi bạn bắt đầu.", author: "Nido Qubein" },
  { text: "Mỗi ngày là một cơ hội để bắt đầu lại. Đừng tập trung vào những thất bại của ngày hôm qua, hãy bắt đầu ngày hôm nay với những suy nghĩ và kỳ vọng tích cực.", author: "Catherine Pulsifer" },
  { text: "Bạn có thể học những điều mới bất cứ lúc nào trong đời nếu bạn sẵn sàng trở thành người mới bắt đầu. Nếu bạn thực sự học cách thích làm người mới bắt đầu, cả thế giới sẽ mở ra với bạn.", author: "Barbara Sher" },
  { text: "Một bình minh mới, một ngày mới, một cuộc sống mới... và tôi cảm thấy tốt.", author: "Nina Simone" },
  { text: "Phép màu trong những khởi đầu mới thực sự là mạnh mẽ nhất trong tất cả.", author: "Josiyah Martin" },
  { text: "Đây là một năm mới. Một khởi đầu mới. Và mọi thứ sẽ thay đổi.", author: "Taylor Swift" },
  { text: "Tương lai bắt đầu từ hôm nay.", author: "Wayne Gerard Trotman" },
  { text: "Hãy tạo ra tương lai của chúng ta ngay bây giờ, và hãy biến ước mơ của chúng ta thành hiện thực của ngày mai.", author: "Malala Yousafzai" },
  { text: "Điều tốt nhất vẫn chưa đến.", author: "Frank Sinatra" },
  { text: "Đó là một bình minh mới, đó là một ngày mới, đó là một cuộc sống mới cho tôi và tôi cảm thấy tốt.", author: "Leslie Bricusse and Anthony Newley" },
  { text: "Sự khởi đầu là phần quan trọng nhất của công việc.", author: "Plato" },
  { text: "Cái mà chúng ta gọi là sự khởi đầu thường là sự kết thúc. Và để kết thúc là để bắt đầu. Sự kết thúc là nơi chúng ta bắt đầu.", author: "T.S. Eliot" },
  { text: "Bạn không thể quay lại và thay đổi sự khởi đầu, nhưng bạn có thể bắt đầu từ nơi bạn đang ở và thay đổi kết thúc.", author: "C.S. Lewis" },
  { text: "Hãy buông bỏ ngày hôm qua. Hãy để hôm nay là một khởi đầu mới và hãy là người tốt nhất có thể, và bạn sẽ đến được nơi Chúa muốn bạn đến.", author: "Joel Osteen" },
  { text: "Chúng ta phải sẵn sàng từ bỏ cuộc sống mà chúng ta đã lên kế hoạch, để có được cuộc sống đang chờ đợi chúng ta. Lớp da cũ phải được lột bỏ trước khi lớp da mới có thể đến.", author: "Joseph Campbell" },
  { text: "Một cuộc hành trình ngàn dặm phải bắt đầu bằng một bước chân.", author: "Lão Tử" },
  { text: "Hãy bắt đầu từ nơi bạn đang ở. Sử dụng những gì bạn có. Làm những gì bạn có thể.", author: "Arthur Ashe" },
  { text: "Cách để bắt đầu là ngừng nói và bắt đầu làm.", author: "Walt Disney" },
  { text: "Đừng ngại từ bỏ cái tốt để đi tìm cái vĩ đại.", author: "John D. Rockefeller" },
  { text: "Tôi thấy rằng tôi càng làm việc chăm chỉ, tôi càng có vẻ may mắn.", author: "Thomas Jefferson" },
  { text: "Thành công là tổng của những nỗ lực nhỏ, được lặp đi lặp lại hàng ngày.", author: "Robert Collier" },
  { text: "Nếu bạn muốn đạt được sự vĩ đại, hãy ngừng xin phép.", author: "Unknown" },
  { text: "Mọi việc diễn ra tốt nhất cho những người tận dụng tốt nhất cách mọi việc diễn ra.", author: "John Wooden" },
  { text: "Để sống một cuộc sống sáng tạo, chúng ta phải mất đi nỗi sợ bị sai.", author: "Unknown" },
  { text: "Nếu bạn không sẵn sàng mạo hiểm những điều thông thường, bạn sẽ phải chấp nhận những điều bình thường.", author: "Jim Rohn" },
  { text: "Hãy tin tưởng vì bạn sẵn sàng chấp nhận rủi ro, không phải vì nó an toàn hay chắc chắn.", author: "Unknown" },
  { text: "Hãy chọn một ý tưởng. Biến ý tưởng đó thành cuộc sống của bạn - nghĩ về nó, mơ về nó, sống với ý tưởng đó. Hãy để bộ não, cơ bắp, dây thần kinh, mọi bộ phận của cơ thể bạn, tràn đầy ý tưởng đó, và chỉ để lại mọi ý tưởng khác một mình. Đây là con đường dẫn đến thành công.", author: "Swami Vivekananda" },
  { text: "Tất cả những giấc mơ của chúng ta đều có thể trở thành sự thật nếu chúng ta có can đảm để theo đuổi chúng.", author: "Walt Disney" },
  { text: "Những điều tốt đẹp đến với những người chờ đợi, nhưng những điều tốt đẹp hơn đến với những người đi ra ngoài và giành lấy chúng.", author: "Unknown" },
  { text: "Nếu bạn làm những gì bạn đã luôn làm, bạn sẽ nhận được những gì bạn đã luôn nhận được.", author: "Unknown" },
  { text: "Thành công là đi từ thất bại này đến thất bại khác mà không mất đi sự nhiệt tình.", author: "Winston Churchill" },
  { text: "Ngay khi con sâu bướm nghĩ rằng thế giới sắp kết thúc, nó đã biến thành một con bướm.", author: "Tục ngữ" },
  { text: "Những doanh nhân thành đạt là những người cho đi chứ không phải là những người lấy đi năng lượng tích cực.", author: "Unknown" },
  { text: "Bất cứ khi nào bạn nhìn thấy một người thành công, bạn chỉ nhìn thấy những vinh quang công khai, không bao giờ là những hy sinh riêng tư để đạt được chúng.", author: "Vaibhav Shah" },
  { text: "Cơ hội không tự nhiên xảy ra, bạn tạo ra chúng.", author: "Chris Grosser" },
  { text: "Đừng cố gắng trở thành một người thành công, mà hãy cố gắng trở thành một người có giá trị.", author: "Albert Einstein" },
  { text: "Những bộ óc vĩ đại thảo luận về ý tưởng; những bộ óc trung bình thảo luận về sự kiện; những bộ óc nhỏ bé thảo luận về con người.", author: "Eleanor Roosevelt" },
  { text: "Tôi không thất bại. Tôi chỉ tìm ra 10.000 cách không hoạt động.", author: "Thomas A. Edison" },
  { text: "Nếu bạn không coi trọng thời gian của mình, người khác cũng sẽ không. Hãy ngừng cho đi thời gian và tài năng của bạn - hãy bắt đầu tính phí cho nó.", author: "Kim Garst" },
  { text: "Một người đàn ông thành công là người có thể đặt một nền móng vững chắc bằng những viên gạch mà người khác đã ném vào anh ta.", author: "David Brinkley" },
  { text: "Không ai có thể khiến bạn cảm thấy thấp kém nếu không có sự đồng ý của bạn.", author: "Eleanor Roosevelt" },
  { text: "Toàn bộ bí quyết của một cuộc sống thành công là tìm ra vận mệnh của một người để làm, và sau đó làm điều đó.", author: "Henry Ford" },
  { text: "Nếu bạn đang đi qua địa ngục, hãy tiếp tục đi.", author: "Winston Churchill" },
  { text: "Những người đủ điên rồ để nghĩ rằng họ có thể thay đổi thế giới, chính là những người làm được điều đó.", author: "Vô danh" },
  { text: "Đừng lên giọng, hãy cải thiện lập luận của bạn.", author: "Vô danh" },
  { text: "Những gì đối với chúng ta là những thử thách cay đắng thường là những phước lành trá hình.", author: "Oscar Wilde" },
  { text: "Ý nghĩa của cuộc sống là tìm ra món quà của bạn. Mục đích của cuộc sống là cho đi nó.", author: "Vô danh" },
  { text: "Khoảng cách giữa sự điên rồ và thiên tài chỉ được đo bằng thành công.", author: "Bruce Feirstein" },
  { text: "Khi bạn ngừng theo đuổi những điều sai trái, bạn sẽ cho những điều đúng đắn một cơ hội để bắt kịp bạn.", author: "Lolly Daskal" },
  { text: "Tôi tin rằng lòng can đảm duy nhất mà bất kỳ ai cũng cần là lòng can đảm để theo đuổi ước mơ của riêng mình.", author: "Oprah Winfrey" },
  { text: "Không có kiệt tác nào được tạo ra bởi một nghệ sĩ lười biếng.", author: "Vô danh" },
  { text: "Nếu bạn không thể giải thích nó một cách đơn giản, bạn không hiểu nó đủ rõ.", author: "Albert Einstein" },
  { text: "Phúc cho những ai có thể cho đi mà không cần nhớ và nhận mà không cần quên.", author: "Vô danh" },
  { text: "Hãy làm một điều mỗi ngày khiến bạn sợ hãi.", author: "Vô danh" },
  { text: "Sống để làm gì nếu bạn không ít nhất cố gắng làm một điều gì đó đáng chú ý.", author: "Vô danh" }
]

// Function to get a random quote for the day
export const getDailyQuote = () => {
  // Use the current date as seed for random selection
  const today = new Date();
  const seed = today.getFullYear() * 10000 + (today.getMonth() + 1) * 100 + today.getDate();
  const randomIndex = seed % quotes.length;
  return quotes[randomIndex];
}

// Function to get random quote for specific emotion
export const getQuoteForEmotion = (emotion) => {
  const quoteIndex = Math.floor(Math.random() * quotes.length);
  return quotes[quoteIndex];
}
  
